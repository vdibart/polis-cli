...
- [foreign-site-widget.md](foreign-site-widget.md) — How does your nav appear on someone else's polis site when you're logged in? Pull the thread through the serve-time placeholder patch (`hosted.go`), the hydrator (`nav.js`), and the companion comment/follow widget that's in every page's static template.
...
4. Link each **public** file by its GitHub path (`github.com/vdibart/polis-cli/blob/main/...`) so the reader can click through. A tour may describe code that is not public (the hosted service, the discovery service), but it names those files in code spans and never links them: a link into source the reader cannot open is a dead end, and the docs link check fails on one.

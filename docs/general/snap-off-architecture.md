...
| **Discovery Service** | `ds.polis.pub` — Fly-hosted Postgres + Hono. | Regional jurisdiction (EU-only DS); private/enterprise DS for closed networks; community-curated DS (only sites in your scene); a minimal reference DS for tests. | Implement the DS HTTP API ([`docs/ds/developer/api-reference.md`](../ds/developer/api-reference.md)) over any storage backend ([`StorageAdapter`](../ds/developer/storage-adapter.md) is the interface). Point sites at your DS via `DISCOVERY_SERVICE_URL`. | The DS HTTP contract is the federation layer. A site can register with multiple DSes; readers can query multiple DSes. |
| **Policy** | Default tenant policies (`cli-go/pkg/policy/policy.go`) + DS operator seed rules (`discovery-service/schema/postgres.sql`). | Want stricter inbound policies (auto-deny from non-followed); want a community moderation regime; want time-of-day rate limits; want an external policy engine plugged in. | Edit `policies/rules.jsonl` (public) or `.polis/policies/rules.jsonl` (private) with [v2 grammar](policy-grammar.md) rules. For DS-level policies, use `/v1/admin/policies`. | Same v2 grammar applies at every layer (Layer 1 inbound, Layer 2 outbound, Layer 3 DS operator). The evaluator is one piece of code. |
| **Background actors** | A toolchain of hosted-runtime actors (drift detection, auto-remediation, signature watchdogs, account/handle lifecycle); plus `pkg/tailor` for self-hosters (multi-version site diagnostic and auto-fixer). | Self-hosters can simply not run a continuous toolchain; enterprises might layer their own monitoring; alternate remediation policies (quarantine instead of auto-delete). | Use `pkg/tailor` for one-shot health/migration runs. Authoring your own actors over the same on-disk surface is straightforward — they all read the same `.polis/` layout. | The actors are runtime choices. The on-disk surface they watch is the protocol; the "always run on a timer" is the polis.pub-specific composition. |
...
**Day 1.** polis.pub's Fly instance goes offline. `handle.polis.pub` subdomains stop resolving. `ds.polis.pub` API returns 5xx.
...
- [security-model.md](security-model.md) — Why the *fixed* protocol surface is fixed (signing, trust chain).
- [policy-grammar.md](policy-grammar.md) — The grammar that survives layer swaps.

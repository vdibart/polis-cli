...
*For* [Developers](../../README.md#building-on-polis) · [Reviewers](../../README.md#reviewing-the-security-and-identity-design) — *Kind* [Concept](../../README.md#kinds-of-page) — *See also* [spec](../../signet/spec/signing-base.md) · [recipe](../../signet/recipes/05-verify-a-site.md)
...
|---|---|---|---|---|
| **CLI binary** | Go CLI (`cli-go/`) leads. **Bash CLI (`cli-bash/`) is the reference implementation** — the one this document points you at when it says *build a new one on the same file format*. Protocol-format changes get a parity attempt in bash; where parity is not feasible the gap is documented in [`implementation-parity.md`](../../cli/implementation-parity.md), never left silent. *(It will be frozen eventually; it is not frozen now.)* | Different language preference; need to embed polis in another tool; want a TUI/GUI/IDE plugin instead of a terminal. | Build any tool that reads/writes the on-disk format under `.polis/` and `content/`. The Go packages in `cli-go/pkg/` are importable; the Bash script is one file. | Same `.polis/` layout, same signing, same DS-registration semantics. Other tools (webapp, polis.pub, third-party clients) work unchanged. |
...
| **Local storage** | `.polis/` + `content/` directories on the operator's machine. | S3 backing for archival sites; IPFS/git-annex for content addressing; compressed snapshots for long-tail. | Keep the directory *convention*; back it with whatever substrate exposes the same path tree over HTTPS. | URL convention (`/posts/YYYYMMDD/slug.html`, from `content/pub.polis.core/post/YYYYMMDD/slug.md`) is married to the directory convention. Change one, remap the other. |
...
| **Theme** | Shipped CSS themes (`vice`, `studio13`, `studio13-nk`, `especial`, `especial-light`, `turbo`, `zane`). `sols` and `stardust` are reserved. | You want a different palette; you want structural overrides (per-template HTML); you want to ship a community theme bundle. | Drop a `<name>.css` into your bundle's `themes/<name>/`. Define the palette the shapes read ([theme contract](themes.md#css-variable-contract)). Declare `compatible_shapes`. See [themes.md](themes.md). | The palette names the shapes read are the only commitment. Themes are independent; shapes are independent; mixing is supported. |
...
| **Discovery Service** | `ds.polis.pub` — managed Postgres + Hono. | Regional jurisdiction (EU-only DS); private/enterprise DS for closed networks; community-curated DS (only sites in your scene); a minimal reference DS for tests. | Implement the DS HTTP API ([`docs/ds/developer/api-reference.md`](../../ds/developer/api-reference.md)) over any storage backend. Point sites at your DS via `DISCOVERY_SERVICE_URL`. | The DS HTTP contract is the federation layer. A site can register with multiple DSes; readers can query multiple DSes. |
| **Policy** | Default tenant policies (`cli-go/pkg/policy/policy.go`) + DS operator seed rules (`discovery-service/schema/postgres.sql`). | Want stricter inbound policies (auto-deny from non-followed); want a community moderation regime; want time-of-day rate limits; want an external policy engine plugged in. | Edit `policies/rules.jsonl` (public) or `.polis/policies/rules.jsonl` (private) with [v2 grammar](../reference/policy-grammar.md) rules. For DS-level policies, use `/v1/admin/policies`. | Same v2 grammar applies at every layer (Layer 1 inbound, Layer 2 outbound, Layer 3 DS operator). The evaluator has two implementations — Go (`cli-go/pkg/policy/`) for sites, and a TypeScript port in the discovery service — held to the same behaviour by parity tests. |
| **Background actors** | Patrol, Medic, Judge, Clerk, Chaplain, Reaper (hosted), plus Tailor (self-hosted). Rosie is different: she is the user's own agent, running inside the user's server under the user's grant. | Self-hosters can simply not run them; enterprises might layer their own monitoring; alternate remediation policies (quarantine instead of auto-delete). | Don't run them, or import the packages (`cli-go/pkg/{patrol,medic,judge,rosie,...}`) into your own runtime. Tailor is a manual invocation. | The actors are *library code* with thin runtime wrappers. Their checks are importable; the "always run on a timer" is the polis.pub-specific choice. See [`actors.md`](actors.md). |
...
| **Markdown + frontmatter as canonical artifact** | Posts are markdown files with a defined frontmatter shape (`published`, `current-version`, `signature`, etc.) at `content/pub.polis.core/post/YYYYMMDD/slug.md`. | The signature covers canonicalized-markdown-plus-frontmatter. Change the format → existing signatures stop verifying. |
...
| **Core content type minimums** | `pub.polis.post`, `pub.polis.comment`, `pub.polis.follow`, etc. each have minimum-required frontmatter fields (e.g. `published`, `current-version`, `signature` on a post). Bundles may *extend* but not *remove* these. | Readers + DS decoders expect these fields. Remove them → content stops flowing across tenants. |
...
Polis artifacts carry a `generator` field (`polis-cli-go/0.67.0`, `my-custom-cli/1.0.0`). Don't pretend to be polis.pub's CLI; be yourself. Version-bump your generator string as you evolve. Makes debugging across implementations humane.
...
- **A tutorial.** If you want to actually run a custom DS or ship a custom theme, the implementation guides (in [`ds/admin/deployment.md`](../../ds/admin/deployment.md), [`themes.md`](themes.md), [`bundles.md`](bundles.md)) cover the how.
